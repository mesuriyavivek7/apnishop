...
import { userColumns, getAllUsers } from '../data/userDataTable';
...
function User() {
    ...
    const fetchData = async ()=>{
       ...
       try{
         ...
         console.log(users)
         setUser(users.data.map((item,index)=>({...item,id:index})))
       }catch(err){
    ...
    <>
      {
        openConfirmPopUp && (
         <div className="fixed z-50 flex justify-center items-center inset-0 bg-black/50">
          <div className="bg-white w-96 rounded-md p-4 flex flex-col">
            ...
            <span>Are you sure to remove this user?</span>
       ...
       <div className='flex h-full flex-col gap-3 md:gap-4'>
         <div className='bg-white custom-shadow rounded-md md:py-4 py-3 px-3 flex items-center justify-between'>
           <h1 className='text-gray-600 text-base md:text-lg font-medium'>User Details</h1>
           <div className='flex items-center gap-3'>
             ...
             <Link to={'/admin/users/addnew'}><button className='md:p-2 p-1.5 bg-themeblue md:text-base text-sm text-white rounded-md'>Add New User</button></Link>
           </div>
         </div>
         <div className='h-full py-4 px-3 custom-shadow rounded-md bg-white'>
           <Box sx={{height:"100%",
            '& .super-app-theme--header': {
             ...
             <DataGrid
              rows={filterUser}
              columns={userColumns(handleOpenConfirmPopUp,handleNavigate)}
              loading={loading}
              initialState={{
              pagination: {
                paginationModel: {
